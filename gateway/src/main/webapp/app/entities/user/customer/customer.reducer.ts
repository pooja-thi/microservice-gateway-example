...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_CUSTOMER: 'customer/PARTIAL_UPDATE_CUSTOMER',
...
export default (state: CustomerState = initialState, action): CustomerState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_CUSTOMER):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_CUSTOMER):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_CUSTOMER):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
const apiUrl = 'services/user/api/customers';
...
export const partialUpdate: ICrudPutAction<ICustomer> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_CUSTOMER,
